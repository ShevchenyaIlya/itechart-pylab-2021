...
def parse_user_page(
    user_profile_info, user_page_url, current_post_info, logger, xpath_templates
):
    try:
        current_post_info["user_karma"] = user_profile_info.select_one(
        ...
        logger.debug(
            f"Failed to access user(username: {current_post_info['username']}, "
...
def parse_reddit_page(
    ...
    try:
        ...
        while parsed_post_count < post_count:
            ...
            total_posts_count += 1
            user_profile_info = get_user_html_from_new_browser_tab(
            ...
            if parse_user_page(
                ...
                logger.debug(
                    f"All information has been received on this post(url: {current_post_info['post_url']})"
                )
                parsed_post_count += 1
            else:
                continue
